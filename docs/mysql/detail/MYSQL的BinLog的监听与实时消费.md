## Mysql Binlog的监听与实时处理工具Maxwell

Maxwell可以做为一个后台进程的应用，读取并解析Mysql的binlogs，同时把结果作为json串写入到kafka,kinesis或其他流式平台，Maxwell操作开锁低，只需要mysql和数据写入的地方

具体见[官方文档(http://maxwells-daemon.io/)](http://maxwells-daemon.io/)

### Mysql开启binlog

```bash
##然后进入/etc/mysql/ 修改my.cnf,增加如下内容
server-id=1
log-bin=master
binlog_format=row
###重启服务 ，确认binlog是否开启
...
```bash
curl -sLo - https://github.com/zendesk/maxwell/releases/download/v1.19.7/maxwell-1.19.7.tar.gz | tar zxvf -
cd maxwell-1.19.7
```
...
#写入文件
producer=file
output_file=/Users/zhangbingbing/Public/maxwell-1.19.7/test.data
...
kafka详细操作见[官方文档](https://kafka.apache.org)  [快速开始](https://kafka.apache.org/quickstart)
